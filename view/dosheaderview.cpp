...
#include "dosheaderview.h"


DosHeaderView::DosHeaderView(QWidget *parent) : QTableView(parent), parent(parent)
{
...
DosHeaderView::~DosHeaderView()
{
    delete m_model;
}

void DosHeaderView::onClickTableRow(const QModelIndex& index)
{
}

void DosHeaderView::initConnect()
{
    connect(m_model, &QStandardItemModel::itemChanged, this, &DosHeaderView::onItemChanged);
}

void DosHeaderView::showEvent(QShowEvent *event)
{
    LOG_DEBUG(__FUNCTION__" is called");
    emit reqUpdateData();
}

bool DosHeaderView::initTableHeader(int nColumn)
{
    // n行m列
...
void DosHeaderView::addData(const QString& member, const QString& offset, const QString& size, const QString& value)
{
...
void DosHeaderView::clearTable()
{
    m_model->removeRows(0, m_model->rowCount());
}

void DosHeaderView::onItemChanged(QStandardItem *item)
{
    if (!item) return;
    ...
    if (item->column() == 3)
    {
        ...
        emit reqModifyDosHeaderValueField(offsetValueStr, newValue);
    }
}
