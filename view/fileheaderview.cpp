...
#include "fileheaderview.h"


FileHeaderView::FileHeaderView(QWidget *parent) : QTableView(parent), parent(parent)
{
...
void FileHeaderView::onClickTableRow(const QModelIndex& index)
{
}

void FileHeaderView::initConnect()
{
    connect(m_model, &QStandardItemModel::itemChanged, this, &FileHeaderView::onItemChanged);
}

void FileHeaderView::showEvent(QShowEvent *event)
{
    LOG_DEBUG(__FUNCTION__" is called");
    emit reqUpdateData();
}

bool FileHeaderView::initTableHeader(int nColumn)
{
    // n行m列
    QStandardItemModel *model = new QStandardItemModel(0, nColumn, this);

    model->setHeaderData(0, Qt::Horizontal, QStringLiteral("Member"));
    model->setHeaderData(1, Qt::Horizontal, QStringLiteral("Offset"));
    model->setHeaderData(2, Qt::Horizontal, QStringLiteral("Size"));
    model->setHeaderData(3, Qt::Horizontal, QStringLiteral("Value"));
    model->setHeaderData(4, Qt::Horizontal, QStringLiteral("meaning"));

    // 居中1: 0, 1, 2, 3 代表列索引
    model->setHeaderData(0, Qt::Horizontal, QVariant(Qt::AlignCenter), Qt::TextAlignmentRole);
...
void FileHeaderView::addData(const QString& member, const QString& offset, const QString& size, const QString& value, const QString& meaning)
{
    ...
    QStandardItem* itemMeaning = new QStandardItem(meaning);
    itemMeaning->setFlags(itemMeaning->flags() & ~Qt::ItemIsEditable);
    rowItems << itemMeaning;
...
void FileHeaderView::clearTable()
{
    m_model->removeRows(0, m_model->rowCount());
}

void FileHeaderView::onItemChanged(QStandardItem *item)
{
    if (!item) return;
