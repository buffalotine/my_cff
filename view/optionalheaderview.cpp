...
#include "optionalheaderview.h"


OptionalHeaderView::OptionalHeaderView(QWidget *parent) : QTableView(parent), parent(parent)
{
...
OptionalHeaderView::~OptionalHeaderView()
{
    delete m_model;
}

void OptionalHeaderView::onClickTableRow(const QModelIndex& index)
{
}

void OptionalHeaderView::initConnect()
{
    connect(m_model, &QStandardItemModel::itemChanged, this, &OptionalHeaderView::onItemChanged);
}

void OptionalHeaderView::showEvent(QShowEvent *event)
{
    LOG_DEBUG(__FUNCTION__" is called");
    emit reqUpdateData();
}

bool OptionalHeaderView::initTableHeader(int nColumn)
{
    // n行m列
    ...
    model->setHeaderData(4, Qt::Horizontal, "Meaning");
...
void OptionalHeaderView::addData(const QString& member, const QString& offset, const QString& size, const QString& value, const QString& meaning)
{
...
void OptionalHeaderView::clearTable()
{
    m_model->removeRows(0, m_model->rowCount());
}

void OptionalHeaderView::onItemChanged(QStandardItem *item)
{
    if (!item) return;
